...
const LanguageSwitcher = () => {
  ...
  return (
    <>
      ...
      <div className="hidden lg:block fixed bottom-8 left-8 2xl:left-[150px] z-50">
        <button onClick={() => setIsOpen(!isOpen)} className="relative group">
          ...
          <div className="relative flex items-center gap-3 bg-purple-900/60 backdrop-blur-xl px-4 py-3 rounded-xl border border-white/20 shadow-xl hover:border-orange-500/50 transition-all">
            ...
            <span className="text-white font-medium min-w-[80px] text-left">{selectedLanguage.lang}</span>
            <svg
        ...
        {/* Dropdown */}
        <div className={`absolute bottom-full left-0 mb-2 w-full transition-all duration-300 ${
      ...
      {/* ── MOBILE / TABLET: clean bottom bar ── */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#0B1E33]/95 backdrop-blur-xl border-t border-white/10 shadow-2xl">
      ...
      {/* Push page content up so bottom bar doesn't cover it on mobile */}
